...
def rgb_to_binary(list):
    binary_list = []
    for i in range(len(list)): binary_list.append(format(list[i],'08b'))
    return binary_list
...
def get_xor(a, b):
    ...
    for i in range(len(a)):
        ...
        for c in range(len(l)): xor += str(l[c])
...
def generate_rgb_list(length):
    generated_rgb_list = []
    for k in range(length): generated_rgb_list.append(random.randint(1, 255))
...
def crypt():
    label = Label(window, text= "Please, choose an image to crypt (5:5)", font='Arial 10 bold')
    label.pack(pady = 20)
    ...
    crypted = Image.new(mode = "RGB", size = (img_size[0], img_size[1]))
    crypted.putdata(rgb_list_chunked)
    crypted.save(get_path('crypted.png'))

    key = Image.new(mode = "RGB", size = (img_size[0], img_size[1]))
    key.putdata(generated_rgb_list_chunked)
    key.save(get_path('key.png'))

    label.destroy()
    label = Label(window, text= "Done. Your crypted image and key image have been saved at " + get_path(''), font='Arial 10 bold')
    label.pack(pady = 20)
...
def uncrypt():    
    label = Label(window, text= "Please, choose an image to uncrypt", font='Arial 10 bold')
    label.pack(pady = 20)
    ...
    label.destroy()
    label = Label(window, text= "Please, choose a key to uncrypt the image", font='Arial 10 bold')
    label.pack(pady = 20)
    ...
    uncrypted = Image.new(mode = "RGB", size = (crypted_size[0], crypted_size[1]))
    ...
    label.destroy()
    label = Label(window, text= "Done. Your uncrypted image has been saved at " + get_path('uncrypted.png'), font='Arial 10 bold')
    label.pack(pady = 20)
...
title = Label(window, text="Image Encryption Tool", bg="black", font=("Arial", 30), fg="white")
title.pack(pady=10)

options = Label(window, text="Options :", bg="black", font=("Arial", 15), fg="white")
options.pack(pady=100)

crypt_btn = Button(window, text='Crypt an image', bd='20', command=lambda:[destroy_main_menu(), crypt()])
uncrypt_btn = Button(window, text='Uncrypt an image', bd='20', command=lambda:[destroy_main_menu(), uncrypt()])
